...
* **Atomic Batching:** Groups compatible datasets into a single `zfs snapshot` invocation for consistent snapshot timestamps with fewer calls.
...
/usr/local/etc/cron.d/diffsnap will be installed automatically. You must uncomment the line for it to begin running on a schedule. Once enabled you can modify it to adjust the default from every minute.
```cron
